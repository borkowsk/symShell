...
/*******************************************/
{
      ...
      XtVaGetValues(item->wid,XmNbackground,&Gray,NULL);
...
get_file_input( XtPointer client_data,int* fid,XtInputId* id)
/************************************************************/
{
...
menu_optcode* mainMenuCode=NULL;/* MAIN MENU CODE */
   ...
   if(argc>1)
    ...
    /* CREATE MENU AND CHILDREN */
          
    mainMenuCode=read_menu_optcodes(stdin);
	
    /*save_menu_optcodes(stdout,mainMenuCode); DEBUG*/
