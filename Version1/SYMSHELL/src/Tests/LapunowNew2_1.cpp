// To niestety nie jest przenoœnie napisane i jak siê okaza³o w BDS wywo³uje
// b³êdy numeryczne i nie rysuje...
//
#include <stdio.h>
#include <conio.h>
#include <ctype.h>
...
// szacowanie funkcji lapunowa przy zmiennej strategi
{
	...
	for(i=1;i<=NUMBER;i++)
	{
		...
		if (wx>gran) { //fprintf(trout,"\n a aktr=%f wx=%f ",aktr,wx);
			return 0.0 ;}
		if (wx<-gran){ //fprintf(trout,"\n a aktr=%f wx=%f ",aktr,wx);
			return 1.0 ;}

		aktr=fabs(aktr-dwa*aktrWx);

		if (aktr>gran) { //fprintf(trout,"\n b aktr=%f wx=%f ",aktr,wx);
			return 0.0 ;}
		...
		if (total>gran) { //fprintf(trout,"\n t aktr=%f total=%f ",aktr,total);
			return 0.0;}
		if (total<-gran){ //fprintf(trout,"\n t aktr=%f total=%f ",aktr,total);
			return 1.0;}
	}

	MAPM ret=(total/(i-1));//end lapunow
	//ret.round(DBL_DIG);//Brak konwersji na double
	char bufor[1000];
	ret.toFixPtString(bufor,DBL_DIG);
	//printf("%s\n",bufor);
	double pom=atof(bufor);	//Straszny to pomysl, ale innego nie mam
	printf("%c",pom>0?'+':'-');
	return pom;
}
#endif
...
// szacowanie funkcji lapunowa przy zmiennej strategi 
{
	...
	return (total/(i-1));//end lapunowBIN
}
*/
/*
...
// szacowanie funkcji lapunowa przy zmiennej strategi 
{
	...
	return (total/(i-1));//end lapunow LN
}
*/
...
void init(void)
{	
	int Koniec=0,precision=20;//Sterowanie zakonczeniem z menu i precyzja obliczen softwarowych
	char bufor[1000];
	char command;
	double X=4.0,Y=4.0;
	...
	{
		//printf("Can't initilize graphics output!\n Press enter to exit!");
...
#ifdef USE_M_APM
	m_apm_cpp_precision(20);//Ustawienie domyslnej precyzji softwarfeowej artmetyki
#endif
	...
	do{
		...
		printf(" \nWcisnij x,X,y,Y,L,P,o,g,d,p lub ENTER !\n>>");
		...
		//case 'p':printf("precyzja=%d",precision);gets(bufor);precision=atol(bufor);m_apm_cpp_precision(precision);break;
		case 'm':
