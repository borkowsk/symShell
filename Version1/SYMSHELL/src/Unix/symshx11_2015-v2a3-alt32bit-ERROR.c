...
static char window_name[1024] = "WB X11 window for simulation shell";
...
#else
static unsigned int default_deph = 24; /* Window deph. 8=>256 colors 32=>true colors*/
#endif
static unsigned int border_width = 4; /* Four pixels */
static unsigned int display_width, display_height;
/* static unsigned int icon_width, icon_height; */ /* Not used, at least currently */
...
static unsigned NumberOfColors = 512; /* Mostly for external use */
static int Scale[514];
...
void fix_size(int Yes) {
    fprintf(stderr, "\nSYMSHX11: %s\n", "Fix_size() not implemented jet - X11 window have always fixed size");
}
...
/* Wymuszenie oczekiwania przez pewn� liczb� ms */
{
      usleep(ms*1000);//useconds_t usec !!!
}
...
/* Definiuje takie kolory jakich byśmy chcieli - niekoniecznie musi się 
 udać wszystkie zaalokować gdy nie true-color! */ {
    ...
    /*  Kolor 0 na pewno ma być czarny */
    RGBarray[0] = RGB;
    ...
    if (UseGrayScale) {
        ...
        for (k = 1; k < 256; k++) {
            ...
            wal = 255 * sin(kat * 1.24);
            if (wal > 0) RGBarray[k].red = wal;
            wal = 255 * (-sin(kat * 0.86));
            if (wal > 0) RGBarray[k].green = wal;
            wal = 255 * (-cos(kat * 1.09));
    ...
    if (trace)
        fprintf(stderr, "%s\n", "SetScale(): Default scale  defined");
}
...
/* inline - jak są inline to w GCC ich nie widać */ 
/* TO JAKOŚ ŻLE DZIAŁA - PSUJE GC ???? TODO */ 
/* Czy to może działać? I w jakich warunkach? */   
/*int buildColorAlpha(unsigned char red, 
                    unsigned char green, 
                    unsigned char blue, 
                    unsigned char alpha) { 
    return ( (int) (alpha) << 24) + 
...
/* Zmienia definicja koloru. Indeksy 0..255 */ {
#ifdef CREATE_FULL
    Scale[color] = buildColor(r, g, b); /* dla 32 bit! */

#else   /* NOT SHURE THAT TRUE COLOR */
    XColor pom;
    ...
    if (XAllocColor(display, colormap, &pom) != 0) {
        ...
        fprintf(stderr, "XAllocColor failed for R%i G%i B%i\n", r, g, b);
    }
#endif
}
...
static void getGC(Window win, GC * gc, XFontStruct * font_info) {
    ...
    *gc = XCreateGC(display, win, 0, &values); //0 means ignore XGCvalues (?) */
    XSync(display, True);
    if (trace)
    ...
    //#else
    colormap = XCreateColormap(display, win, MyVisual /*DefaultVisual(display,screen_num)*/, AllocNone);
    //#endif
    XSync(display, True);
    if (trace)
    ...
    XSync(display, True);
    if (trace)
        fprintf(stderr, "XSetWindowColormap OK\n");


#ifndef  CREATE_FULL        
    pomColor.red = pomColor.green = pomColor.blue = 0;
    Black = pomColor.pixel;
    Scale[0] = pomColor.pixel;

    XAllocColor(display, colormap, &pomColor);
    XSync(display, True);
    if (trace)
        fprintf(stderr, "XAllocColor for Black OK\n");

    pomColor.red = pomColor.green = pomColor.blue = 0xffff;
    White = pomColor.pixel;
    Scale[255] = pomColor.pixel;
...
#ifdef  CREATE_FULL
    for (i = 0; i < 512; i++) /* TODO CHECK */ {
        pomColor = Array[i];
        Scale[i] = buildColor(pomColor.red & 0xff, pomColor.green & 0xff, pomColor.blue & 0xff);
    }
#else
    for (i = 0; i < 512; i++) {
        ...
        if ((ret = XAllocColor(display, colormap, &pomColor)) != 1) {
            if (trace)
            ...
            if (trace)
                fprintf(stderr, "! XAllocColor #%u return %d\n", i, ret);
        }
        //fprintf(stderr," %d ",ret);
        Scale[i] = pomColor.pixel; //Tu się zmienia kolor w skali, jak tryb indeksowany
    }
    ...
    /* Czy to potrzebne? Tak na wszelki wypadek */
...
/* typowa dla platformy inicjacja grafiki/semigrafiki
 a,b to wymagane rozmiary ekranu */ {
    ...
    if (!XMatchVisualInfo(display, XDefaultScreen(display), 32, TrueColor, &MyVInfo)) {
        ...
        fprintf(stderr, "Matched visual 0x%lx class %d (%s) depth %d (user requested %d)\n",
                MyVInfo.visualid,
                MyVInfo.class,
     ...
     * properly */
    {
        char* pom[4] = {window_name, "\0", icon_name, "\0"}; /* Poniższe funkcje "chcą" danych w takiej tablicy */
        if (XStringListToTextProperty(pom, 1, &windowName) == 0) {
        ...
        if (XStringListToTextProperty(pom + 2, 1, &iconName) == 0) {
            fprintf(stderr, "%s: structure allocation for "
                    "iconName failed.\n", progname);
            exit(-1);
            return 0;
    ...
    if (signal(SIGHUP, SigPipe) != SIG_ERR && trace)
        fprintf(stderr, "SIGHUP handler installed\n");

    if (signal(SIGINT, SigPipe) != SIG_ERR && trace)
        fprintf(stderr, "SIGINT handler installed\n");

    while (!input_ready()); /* Wait for expose */

    /* Czysci zeby wprowadzic ustalone tlo */
    if (trace)
        fprintf(stderr, "init_plot() finished. Background is %d\n", (int) bacground);
...
static void CloseAll()
/* Zwalnia wszystko co przypięte do "display" */ {
    if (trace)
        fprintf(stderr, "CloseAll() - removing X11 resources:  \n");

    if (display == 0) {
        fprintf(stderr, "Traing to free resources of NULL display!\n");
        return;
    }
    ...
    if (colormap != 0) {
        ...
        XSync(display, True);
        if (trace)
            fprintf(stderr, "Colormap uninstaled and unloaded\n");
    }
#endif
    ...
    if (alloc_cont != 0) {
        ...
        if (trace)
            fprintf(stderr, "%s %p\n", "FREE PIXMAP", (void*) cont_pixmap);
        cont_pixmap = 0;
    }

    if (win != 0) {
        if (trace)
            fprintf(stderr, "%s %p ", "DESTROY WINDOW", (void*) win);

        XDestroyWindow(display, win);
        if (trace)
            fprintf(stderr, "-OK win: %p\n", (void*) win);
        win = 0;
    }
...
/* Alokuje pixmape na zawatosc okna */ {
    ...
    if (trace) {
        XFlush(display);
        fprintf(stderr, "PIXMAP %p DISPLAY %p WIN %p %s\n",
                (void*) cont_pixmap,
                (void*) display,
                (void*) win,
                "OK?");
...
REPLAY:
    ...
    if ((l_font_info = XLoadQueryFont(display, fontname)) == NULL) {
    ...
    } else /* Probujemy odczytać nazwe i właściowści fontu */
        if (trace) {
        ...
        if (XGetFontProperty(l_font_info, XA_FONT_NAME, &card32)) {
            ...
            /* fam = XtNewString (fn); PO CO?  TODO
               strtolower (fam); */
            fprintf(stderr, "FONT   NAME: `%s'\n", fn);
            XFree(fn); /* Było potrzebne tylko na chwilę */
        }
...
int char_height(char znak) /* TODO - jest prowizorka */ {
    int pom0 = XTextHeight(font_info); /* Czy to nie działa z dziwnych powodów? TOCHECK */
    int pom = (font_height + muly) / muly; /* I po co to dodawanie i dzielenie? Chyba żeby 0 nie wyszło? */
    if (pom < 1)
        pom = 1;
    return pom;
}
...
/* BARDZO PRYMITYWNIE! */
/* A aktualne rozmiary lancucha? Inne znaki niż pierwszy? */ {
...
/* --------//---------- uzgodnienie zawartosci ekranu */ {
    ...
    if (animate) {/* Wyswietlenie bitmapy */
        XCopyArea(display, cont_pixmap, win, gc,
                0/*src_x*/, 0/*src_y*/,
                width, height,
                0/*dest_x*/, 0/*dest_y*/);
...
/* zalezna od platformy funkcja sprawdzajaca czy jest wejscie */ {
    ...
    if (XPending(display) != 0) /* Sprawdzenie czy nie cdma zdarzen */ { /*SA JAKIES!*/
...
/* Wlacza drukowanie tekstu bez zamazywania t�a. Zwraca stan poprzedni */ {
    int ret = _transparently; /* ALE BEZ KANAŁU ALfa I TAK TU NIE MA JAK ZADZIAŁAĆ */
    _transparently = yes;
    return ret;
}
...
/* ---//--- wyprowadzenie tekstu na ekran */ {
    ...
    if (_transparently)
        XSetBackground(display, gc, buildColor(205, 205, 255)); /* buildColorAlfa i tak nie działa mimo trybu 32 bit a rozwala jakoś GC TODO */
    else
...
void printc(int x, int y, ssh_color fore, ssh_color back,
        const char* format, ...)
/* Drukuje w kolorach uzytkownika, ale wzietych z palety*/ {
    size_t len = 0;
    unsigned font_height = 0;
    ...
    if (_transparently)
        XSetBackground(display, gc, buildColor(205, 205, 255)); /* buildColorAlfa i tak nie działa mimo trybu 32 bit a rozwla jakoś GC TODO */
    else
...
void fill_rect_d(int x1, int y1, int x2, int y2) {/* Wyswietla prostokat w kolorach domyslnych - czyli taki pędzel jaki został */
    x1 *= mulx; /* Multiplicaton of coordinates */
    y1 *= muly; /* if window is bigger */
    x2 *= mulx; /* Multiplicaton of 2' coordinates */
    y2 *= muly; /* if window is bigger */
...
/* Ustala styl rysowania lini: SSH_LINE_SOLID, SSH_LINE_DOTTED, SSH_LINE_DASHED */ { /* SSH_LINE_SOLID  1   SSH_LINE_DOTTED 2  SSH_LINE_DASHED 3 */
    extern int basic_line_width; /* Z jakiegoś powodu GCC nie widzi że to jest zadeklarowane powyżej */
...
/* wyswietlenie lini */ {
    ...
    x1 *= mulx;
    x2 *= mulx; /* Multiplicaton of coordinates */
    y1 *= muly;
    y2 *= muly; /* if window is bigger */
    if (c != CurrForeground) {
        CurrForeground = c;
...
void line_d(int x1, int y1, int x2, int y2) {/* Wyswietlenie lini w kolorze domyslnym - czyli wcześniej ustawionym "pędzlem"*/
...
/* Wyswietlenie okregu w kolorze c */ {
    ...
    x *= mulx;
    y *= muly; /* Multiplication of coordinates */
    r1 = r*mulx;
    r2 = r*muly; /* if window is bigger */
    if (c != CurrForeground) {
        CurrForeground = c;
        XSetForeground(display, gc, Scale[c]);
    }
...
/* --------//---------- zamkniecie grafiki/semigrafiki */ {
    ...
    if (opened) {
        inside_close_plot = 1;
        if (trace) {
            fprintf(stderr, "close_plot() called! ");
        }

        retXsync = XSync(display, True/*DISCARD PENDING EVENTS*/);
        ...
        if (WB_error_enter_before_clean) {
            ...
            fflush(stderr);
            fflush(stdout);
            int ret = XSync(display, True/*DISCARD EVENTS*/);
            get_char();
...
void shell_setup(/* const char* title, int iargc, char* iargv[] STARE */
        const char* title, int iargc, char* iargv[]
        /* const char* title, int iargc, const char* iargv[] JESZCZE STARSZE */
        ) {
    ...
    sprintf(progname, "%s\000\0", largv[0]); /* ,1024 other lenght? */
    sprintf(window_name, "%s\000\0", title); /* other lenght? */
    sprintf(icon_name, "%s\000\0", title); /* other lenght? */
...
/* Write "virtual" copy of the  screen to file
 * in format native to platform: BMP, XPM etc. */ {
    char NameBufor[2048];
    sprintf(NameBufor, "%s.xpm", Filename);
    XSync(display, True);

    if (cont_pixmap) {/*  int XpmWriteFileFromPixmap ( 	
         //     Display *  	display,
	 //	char *  	filename,
	 //	Pixmap  	pixmap,
	 //	Pixmap  	shapemask,   //???       
...
/*  f.e:
   if(XWriteBitmapFile(display, Filename, cont_pixmap, width, height, -1, -1)!=BitmapSuccess) //B&W only
 */
