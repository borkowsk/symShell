...
static int ScreenClip=1;/* Czy przycinak okno do wielkości ekranu? */
 ...
 static unsigned defaut_line_width=0;
...
if(win!=0)
    {
    if(trace)
	   fprintf(stderr, "%s %p ","DESTROY WINDOW",win);
...
/* Create pixmap of depth 1 (bitmap) for icon */
    ...
    XSetWMProperties(display, win, &windowName, &iconName,
          ...
          class_hints); //TODO "passing argument 5 from incompatible pointer type"
   /* } ??? */
