...
// !!!:	Zawartoœæ jest "sztefetowana" pomiedzy obiektami powy¿szych typów -
...
#ifdef __BCPLUSPLUS__ //Ma k³opoty w postaci masy warningów "code has no effect"
#pragma warn -8019
#endif
...
public:
explicit wb_sptr(PointedType* ini=NULL):ptr(ini){} //Przejmuje wskaŸnik
explicit wb_sptr(wb_sptr& nini):ptr(nini.give()){} //Sztafetuje wskaŸnik z nini
void dispose(); //Zwalnia zaalokowan¹ pamiêc
...
wb_sptr& operator = (PointedType* nini); //Przypisanie c-wskaŸnika
...
//Dostêp
PointedType* get_ptr_val() const;//Dostêp wartoœci do c-wskaŸnika - trzeba uwa¿aæ
PointedType& operator* () const; //Dostêp do wskazywanej zawartoœci
};
...
{
public:
explicit wb_ptr(PointedType* ini=NULL):wb_sptr<PointedType>(ini){} //Przejmuje wskaŸnik
...
//Przykrycie tych metod które zwracaj¹ wb_sptr<>, ¿eby zwraca³y wb_ptr
wb_ptr& operator = (PointedType* nini); //Przypisanie c-wskaŸnika odpowiedniego typu
...
//Jeszcze jedno jawne przekazywanie wskaŸnika, tyle ¿e zwracaj¹ce wartoœæ. Mo¿e gdzieœ u¿ywane
...
//Klasa z rodziny wb_wskazników przeznaczona na uchwyt do lancuchow tekstowych
...
void finalise() { dispose(); } //Wywolanie rownowa¿ne destruktorowi
void dispose() //Zwalnia zaalokowan¹ pamiêc. Jak u¿yje dziedziczonego to siê pewnie nic nie stanie, ale...
	{ if(ptr) delete [] ptr; ptr=NULL;/*dla sprawdzeñ*/}
...
void alloc(size_t s);// Alokacja do nowego rozmiaru, utrata starej zawartoœci!
size_t get_size() const; //D³ugoœæ ³añcucha w wersji C
const char* get() const; //Skrócone get_ptr_val();
...
//A la C-owe stringi - to co mo¿na bezboleœnie
...
public:
	...
	wb_dynarray(const wb_dynarray& nini):size(nini.size),ptr(nini.ptr)//!!! Mo¿e siê zdarzyæ, ¿e kopiujemy pust¹!
...
//Transfer czyli przêjêcie zawartoœci od parametru typu wb_dynarray
...
size_t expand(size_t s,const T& FillVal); //Rozszerzenie wektora - przydatne do rozbudowy tablicy
										  ...
										  //podobnie jak poni¿sze metody, które te¿ opakowuj¹ memcpy
size_t trunc(size_t s);//Skrócenie wektora - przydatne przy czytaniu (umo¿liwia oszczêdnoœæ pamiêci)
void copy_from(const T Where[],size_t how_many_elements);//Kopiowanie zawartoœci
void copy_from(const wb_dynarray<T>& Where); //Kopiowanie zawartoœci
};
...
template<class PointedType> inline
wb_ptr<PointedType>& wb_ptr<PointedType>::operator = (PointedType* nini) //Przypisanie wskaŸnika odpowiedniego typu
	{
	return set(nini);
	}
...
//	delete (void*)ptr; //?Zwalnianie bez wywo³ywania mo¿liwych destruktorów
...
{																				WBPTRLOG( "wb_dynnarray::expand("<<s<<")" )
	if(ptr==NULL) //Bo wtedy bzdura
			return 0;														   	assert(s>size); //Powiêkszamy!
	T* tmp=new T[s];

	//	memcpy(tmp,ptr,s*sizeof(T));
	//	delete (void*)ptr; //?Zwalnianie bez wywo³ywania mo¿liwych destruktorów

	//Bezpieczne choæ nieefektywny przepisanie zawartoœci komórek
	for(unsigned i=0;i<size;i++)
		tmp[i]=ptr[i]; //Dla klas uzyty operator przypisania

	//Jakieœ wype³nienie nowych komórek
...
//U¿ywa memcpy ale niczego nie likwiduje wiêc to chyba nie problem
template<class T> inline
...
//Ale musia³by mieæ zawsze dostêp do randomizera, a zastosowanie jest rzadkie raczej
{
	char bufor[sizeof(T)];														assert(ptr!=NULL);
																				assert(index<size);
	if(index>=size-1) return;//Nic nie trzeba robic
...
/////////////////////////////////////
//Operacje ³añscuchowe, wypelnianie tablic i macierzy
//////////////////////////////////////////////////////////////
...
//Iplementacja zapisu lancucha znaków na strumien ze sprawdzaniem czy trzeba w \"
