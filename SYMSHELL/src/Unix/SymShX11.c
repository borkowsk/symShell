...
/* WERSJA Z POPRAWIONYM EXPOSE - ALE NIE PRZETESTOWANA  */
#include <stdlib.h>
#include <stdio.h>
#include <stdarg.h>
#include <signal.h>
 ...
 * is global because it is needed in most application functions */ 
 ...
 static int repaint_flag=0; /* Czy user ma co przeczytac przez repaint_erea */
...
unsigned  get_fixed()
/* Zwraca informacje o sposobie 
obslugi zmiany rozmiarow okna */
{
return 1;
}
...
if(trace)
	fprintf(stderr,"buffering_setup(animate=%d)"
		       "buffered=%d\n",animate,buffered);
}
...
#ifndef __MSDOS__ /* SYMULATOR X-LIB pod DJGPP */
if(colormap!=0)
	{
 ...
 /* Get events, use first to display text and graphics */ 
  ...
  case Expose: 
  {
    ...
    if( (!buffered) || buffer_empty )
		...
		repaint_flag=1;/* Sa juz dane dla repaint_area - jesli user przeczyta to sie
		skasuje*/
		...
		if(report.xexpose.count == 0 || buffer_empty  )
			...
			if(trace)
		  		fprintf(stderr,"EXPOSE force repaint "
						"(buffered=%d buffemp=%d)\n",
				buffered,buffer_empty);
			buforek[0]='\r';
			buffer_empty=0;
			}
		}
...
for(i=1;i<largc;i++)
	...
	if(strncmp(largv[i],"-tracemen",9)==0)
		{
		ssh_menu_trace=(largv[i][9]=='+')?1:0;
	    fprintf(stderr,"Trace menu is %s\n",(trace?"ON":"OFF"));
		}
	}
}
...
do{
Read_XInput();
} while(buforek[0]==NODATA); /*Moze przerabiac zdarzenia ktore nie daja znaku!*/
pom=buforek[0];
buforek[0]=NODATA;
...
if(repaint_flag)
	...
	*height=last_repaint_data.height/muly;
	...
	repaint_flag=0;/* USER PRZECZYTAL */
	return 0;
	}
	else
	return -1;	
